            ...
            if (OS_SEM_TAKE_CHK(os_sem_take(SSpiRW[pSSpiRW->ucUnit].SemRx, OS_T_FOREVER)))
            {
               ...
               for (bLock= FALSE, bMatch= FALSE, bNew= FALSE, usItem= 0; usItem < usItemCnt; usItem++)
               {
                  ...
                  if (SSpiRO[pSSpiRW->ucUnit].pBase->SPI_MR & AT91C_SPI_MSTR)
                  {
                     ...
                     ucChannel= (SSpiRO[pSSpiRW->ucUnit].pBase->SPI_MR & AT91C_SPI_MSTR) ? (SPI_CHANNELID_MAX) : (0);
                     usResult2= circular_read(ucSpiAddrRx[pSSpiRW->ucUnit], &ucChannel, 1);
                  }
                  else
#endif
                  {
                     // This information is sporadically set wrong by spi pdc (esp. in master mode)
                                             ...
                                             if (circular_freeWrite(usSpiChannelCntRxQ[pSSpiRW->ucUnit][SSpiChannelMapRW[pSSpiRW->ucUnit][ucChannelCurr].ucStorageIdx]) >= 1)
                                             {
                                                if (circular_freeWrite(ucSpiChannelTagRx[pSSpiRW->ucUnit][SSpiChannelMapRW[pSSpiRW->ucUnit][ucChannelCurr].ucStorageIdx]) >= 1)
                                                {
                                                   if (circular_freeWrite(ulSpiCntTx[pSSpiRW->ucUnit]) >= 1)
                                                   {
                                                      ...
                                                      if ((SSpiRO[pSSpiRW->ucUnit].pBase->SPI_MR & AT91C_SPI_MSTR) ? (circular_freeWrite(ucSpiAddrRx[pSSpiRW->ucUnit]) >= 1) : (TRUE))
                                                      {
#endif
                                                         // Putting partial data multiple times is not supported because it's prevented by LASTXFER-BUG!!!
                                                         if (circular_freeWrite(ulSpiTx[pSSpiRW->ucUnit]) >= usBytesPerXferJob)
                                                         {                                                         
                                                            // Set length for next rx cycle (starts simultaneously together with this tx as soon as started)
                                                            usResult= circular_write(usSpiChannelCntRxQ[pSSpiRW->ucUnit][SSpiChannelMapRW[pSSpiRW->ucUnit][ucChannelCurr].ucStorageIdx], (USHORT *)&usBytesPerXferJob, 1);                                       
                                                            if (usResult > 0)
                                                            {
                                                               ...
                                                               if (usResult > 0)
                                                               {
                                                                  ...
                                                                  if (usResult > 0)
                                                                  {
                                                                     // Determine bytes per pdc item for the current channel
                                                                     ...
                                                                     if (usResult > 0)
                                                                     {
                                                                        ...
                                                                        do
                                                                        {
                                                                           // Clear all item bits (data maybe only partially overwritten)
                                                                           ulPDCItem= 0;
                                                                           
                                                                           // Read plain data item from channel buffer into item
                                                                           usResult= circular_read(ucSpiChannelTx[pSSpiRW->ucUnit][SSpiChannelMapRW[pSSpiRW->ucUnit][ucChannelCurr].ucStorageIdx], (UCHAR *)&ulPDCItem, usBytesPerPDCItem);
                                                                           if (usResult > 0)
                                                                           {                                         
                                                                              usByteCount+= usBytesPerPDCItem;
                                                                              ...
                                                                              // Write one pdc item (8-16 bit) to unit buffer
...
void SpiIrqHandler(UCHAR ucUnit)
{
   ...
   // Rx double bank pdc counters are zero after startup
   if (ulSpiIrq & AT91C_SPI_RXBUFF)
   {
      for (ucChannel= 0; ucChannel < SSpiRW[ucUnit].SCircular.ucSize; ucChannel++)
      {
         // Queue pdx rx bank
   ...
   if (ulSpiIrq & AT91C_SPI_TXBUFE)
   {
      ...
      if (SSpiRW[ucUnit].ucIrqTxState == SPI_TXSTATE_START_TX)
      {
         ...
         do
         {
            ...
            if (usResult)
            {
               ...
               usBytesPerPDC= (ulJobDetail & BITS_ULONGHL) >> BITS_OF(USHORT);
               usItemsPerJob= (usBytesPerPDC) ? (usBytesPerJob/ usBytesPerPDC) : (usBytesPerJob);
         ...
         if (usItemsPerPDC)
         {
            ...
            {            
               ...
               // Trigger TskSpiTx for post processing
      ...
      // Stop pending rx (force tx/rx/tx/rc ... sequence)
