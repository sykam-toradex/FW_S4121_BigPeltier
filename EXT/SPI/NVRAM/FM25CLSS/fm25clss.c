...
// 2017-09-07 fixed data integrity checking for active multistore in Fm25clssReadSafe
// 2017-09-07 fixed data destination calculation for pvDest in Fm25clssReadSafe
//----------------------------------------------------------------------------
               ...
               if (OS_SEM_TAKE_CHK(os_sem_take(SFm25clssRW[ucUnit].SemStart, u16Timeout)))
               {
                  ...
                  {                 
                     ...
                     for (ucCycle= 0; ucCycle < ucCycleMax; ucCycle++)
                     {
                        ...
                        // Split transfers larger than maximum spi block size
                        for (usBlockIdx= 0; (usBlockIdx < USHRT_MAX) && (usToDoCnt); usBlockIdx++)
                        {
                           ...
                           if (usResult >= usByteCnt)
                           {
                              ...
                              {
                                 // Store data always continuously at pvData, all copies continuously at pvTemp (when available)
                                 pvDest= (!ucCycle || SFm25clssRO[ucUnit].bMultiTransfer) ? (((UCHAR *)pvData)+ (usCount* ucCycle)+ ((SPI_PDCMAX- FM25CLSS_SIZE_HEADERMEM- FM25CLSS_SPI_FIX)* usBlockIdx)) : (((UCHAR *)pvTemp)+ (usCount* (ucCycle- 1))+ ((SPI_PDCMAX- FM25CLSS_SIZE_HEADERMEM- FM25CLSS_SPI_FIX)* usBlockIdx));
                     ...
                     // Compare and count differences (for ucCycle > 0 && ucCycleMax > 1 && SFm25clssRO[ucUnit].ucMultiStore > 1 && !SFm25clssRO[ucUnit].bMultiTransfer && pvTemp)
                     if ((ucCycle) && (ucCycleMax > 1))
                     {
                        if ((!SFm25clssRO[ucUnit].bMultiTransfer) && (SFm25clssRO[ucUnit].ucMultiStore > 1))
                        {
                           if (pvTemp)
                           {
                              ...
                              {
                                 // Data layout example for usCount= 3 and SFm25clssRO[ucUnit].ucMultiStore= 4
                                 //         0  1  2  3  4  5  6  7  8
                                 // pvData: a0 b0 c0
                                 // pvTemp: a1 b1 c1 a2 b2 c2 a3 b3 c3
                                 
                                 // Data read
                                 for (ulPos= 0; ulPos < usCount; ulPos++)
                                 {                                    
                                    // Compare with the copies of MultiStore read
                                    for (ulPos2= 0; ulPos2 < (SFm25clssRO[ucUnit].ucMultiStore- 1); ulPos2++)
                                    {
                                       ulDiffCnt += (*((UCHAR *)pvData+ ulPos) != *((UCHAR *)pvTemp+ ulPos+ (usCount* ulPos2)));
                                    }
                                 }
                              }
                              else
